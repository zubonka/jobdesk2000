...
exports.handler = async (event) => {
    ...
    try {
      const resp = await fetch(
        ...
        { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": apiKey },
          body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 500, responseMimeType: "application/json", responseSchema: schema } }) }
      );
      ...
      // прибираємо markdown-огортку, якщо є
      out = out.replace(/```json/gi, "").replace(/```/g, "").trim();
      let parsed = null;
      // 1) пряма спроба
      try { parsed = JSON.parse(out); } catch (e) {}
      // 2) якщо не вийшло — витягуємо перший { ... } блок з тексту (Gemini міг додати пояснення)
      if (!parsed) {
        const m = out.match(/\{[\s\S]*\}/);
        if (m) { try { parsed = JSON.parse(m[0]); } catch (e) {} }
      }
      if (!parsed || typeof parsed !== "object") { lastErr = "Gemini повернув неструктуровану відповідь"; continue; }
      // нормалізуємо поля, щоб фронтенд завжди отримав повний набір
      const clean = {
  ...
  return { statusCode: 200, headers: CORS, body: JSON.stringify({ error: lastErr || "Не вдалося розібрати вакансію" }) };
};

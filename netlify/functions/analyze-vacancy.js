...
exports.handler = async (event) => {
  ...
  let url = "";
  try { url = (JSON.parse(event.body || "{}").url || "").toString(); } catch (e) {}
  if (!url || !/^https?:\/\//.test(url)) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: "Некоректний URL" }) };

  // 1. Завантажуємо сторінку
  let html = "";
  try {
    const pageResp = await fetch(url, {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; JobDeskBot/1.0)" },
      redirect: "follow",
    });
